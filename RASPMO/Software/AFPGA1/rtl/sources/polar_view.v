...
//
// Colour carries FREQUENCY, not amplitude - amplitude is already the ray's
// length, so colouring by it says nothing twice. Frequency is the axis the
// polar view otherwise throws away: it collapses 1024 bins onto one bearing, so
// without it a ray tells you something is over there but not what. Now the
// colour reads straight across to the peak's position in the spectra above.
//
...
//
// Off both masks the colour outputs go black, but they're don't-care - nothing
// downstream looks. Never claim a whole pane opaque here, that just blanks a
// spectrum.
//
...
//
// That feed only means anything during active video. Through blanking active_x
// sits at 0, so the pipeline gets primed with pixel LOOKAHEAD's coordinates over
// and over and those stale results fall out across the FIRST LOOKAHEAD pixels of
// the next line -> a LOOKAHEAD-wide stripe down the left edge wherever the stale
// radius lands inside the disc.
// Nasty because whether you SEE it depends on the centre: at CX=1440 the stale
// radius is 1419 and nothing shows, at CX=64 (testbench) it's 47 against R_MAX
// 48 and the whole stripe lights up. So both masks are qualified with i_video_de
// AS IT WAS AT FEED TIME (de_fed_sr) instead of by some geometry argument, and
// the module draws nothing rather than garbage. Costs the leftmost LOOKAHEAD
// columns, which no sane centre/R_MAX goes anywhere near.
//
