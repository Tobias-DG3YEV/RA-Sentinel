...
//
// NO IDELAYE2 on the DCLK/FCLK clock paths - the clock is clean here and
// IBUFDS_DIFF_OUT's true/complement outputs go straight in as the two ISERDES
// clock phases. Don't add delay taps there to "fix" skew; that only ever
// papers over a termination problem. The IDELAYE2 in the DATA path stays: it
// is not a skew workaround, it is the only way to drive an ISERDESE2
...
/* EVEN and ODD each capture alternating serial bit-times, so the 12 bit word
   ...
   is the electrical INVERSE of the true serial bit and has to be re-inverted
   here in fabric, since these pins are wired true differential polarity.
   Drop this and a static line reads 0xAAA instead of 0x000, while a ramp comes
   out XOR'd with 0xAAA - ramp_checker has no XOR, so it just screams. */
