...
/* Grid rows at ampThresh = 0, 27, 54, ... - a few fixed constants, so this
   decodes to a comparator tree, ~3 LUT levels. Do NOT write it as
   "(ampThresh % 27) == 0": at 1080p that synthesises to a divider network of 4
   carry chains and becomes the design's critical path. Loop bound comes from
   SPEC_H so the grid follows the pane geometry. */
...
wire gridActive;
/* Horizontal grid every 27px = 10.15dB per division at 1px per logfn count
   (0.376 dB/px). Anchored to the BOTTOM of each pane's spectrum (ampThresh,
   NOT pane_y) so the baseline is itself a grid line and divisions read
   0/10/20/... dB above it. ampThresh is an unsigned 9-bit row constant, so no
   signed-wrap hazard. Vertical grid every 32px, pane-local, so all four
   panes line up. */
