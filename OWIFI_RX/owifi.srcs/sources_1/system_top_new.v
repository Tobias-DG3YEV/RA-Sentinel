...
reg [`OUTSTREAM_META_BITS-1:0] addr_out_meta;
reg ram_in_hdr_stb;
reg ram_in_meta_stb;
wire [7:0] meta_out;
wire [7:0] hdr_out;
...
always @(negedge master_clk) begin
    if(reset_system == 1) begin
        ...
        if(addr_out_hdr == (`OUTSTREAM_HDR_SIZE)) begin //reached end of frame buffer?
            dcmi_frame <= 0;
        end
    end
end
    ...
    if(reset_system == 1) begin
        ...
        if(dcmi_frame == 1) begin // idle state
            ...
            if (o_byteOutStrobe == 0) begin
                ...
                if(addr_out_meta == 0) begin
                    o_DCMI_vsync <= 0; // set frame active signal
                    o_byteOut <= 8'h5A;
                    addr_out_meta <= addr_out_meta + 1;
                end
                else if(addr_out_meta == 160 && o_DCMI_hsync == 0) begin
                    o_DCMI_hsync <= 1;
                end
                else if(addr_out_meta != (`OUTSTREAM_META_SIZE - 1)) begin
                    o_DCMI_hsync <= 0;
                    o_DCMI_vsync <= 0; // set frame active signal
                ...
                else if(addr_out_hdr != (`OUTSTREAM_HDR_SIZE - 1)) begin
                    //o_byteOutStrobe <= 1;  // generate pixel clock
